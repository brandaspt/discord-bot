...
Deno.test(
  "postToDiscord sends correct method, headers, and payload",
  withWebhookUrl(async () => {
...
Deno.test(
  "postToDiscord truncates messages longer than 1950 chars",
  withWebhookUrl(async () => {
...
Deno.test(
  "postToDiscord does not truncate messages at exactly 1950 chars",
  withWebhookUrl(async () => {
...
Deno.test(
  "postToDiscord throws on non-2xx response",
  withWebhookUrl(async () => {
    ...
    try {
      await assertRejects(() => postToDiscord("hello"), Error, "Discord webhook failed (401)")
    } finally {
      fetchStub.restore()
    }
  })
